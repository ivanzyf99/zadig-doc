...
关于镜像版本回退，更多信息可阅读：[工作流的镜像版本回退](/dev/workflow/image/rollback/)。
...
- 支持配置多个通知
- 支持通知到指定人（钉钉通过手机号、企业微信和飞书通过 UserID /用户 ID）<Badge text="企业版" />
...
若要将通知指定要人，需获取对应用户的 用户ID，并在 Zadig 中完成配置
用户 ID 可以通过 飞书管理后台->成员与部门，获取 用户 ID

![用户 ID 获取](../_images/lark_getuserid.png)
